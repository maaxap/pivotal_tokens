...
from pivotal_tokens.extractor import SuccessProbabilityShiftSentenceExtractor, THINKING_END_TOKEN
...
class Config:
    ...
    output_filepath: str = field(metadata={"help": "Path to output CSV with identified pivotal sentences."})
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Extract SPS pivotal sentences from traces using a JSON config file.")
    parser.add_argument("--config", type=Path, required=True, help="Path to JSON file containing configuration.")
    return parser.parse_args()



def wipe_off_thinking_end_token(trace: str) -> str:
    wiped_off_trace = trace
    if trace.endswith(THINKING_END_TOKEN):
        wiped_off_trace = trace[:-len(THINKING_END_TOKEN)]
    return wiped_off_trace.strip()
...
def main(config: Config):
    ...
    traces_df["trace"] = traces_df["trace"].apply(wipe_off_thinking_end_token)
    ...
    extractor = SuccessProbabilityShiftSentenceExtractor(model=model,
                                                         tokenizer=tokenizer,
                                                         oracle=oracle,
                                                         base_repo=base_repo,
    ...
    for sample in samples:
        ...
        try:
            pivotal_sentences = extractor.extract(sample_id=sample.id,
            ...
            for sentence in pivotal_sentences:
                accumulator.append(asdict(sentence))
    ...
    logging.info(f"Saved extracted pivotal sentences to {output_filepath}.")
